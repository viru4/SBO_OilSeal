...
export async function sendWhatsApp(to: string, body: string) {
  ...
  if (!sid || !token || !from) {
    throw new Error("Twilio WhatsApp not configured (set TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_WHATSAPP_FROM)");
  }
  const client = twilio(sid, token);
  await client.messages.create({ from, to: to.startsWith("whatsapp:") ? to : `whatsapp:${to}`, body });
}
